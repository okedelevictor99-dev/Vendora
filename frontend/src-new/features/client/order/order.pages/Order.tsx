...
const Order = () => {
  ...
  const { data, isLoading, isError } = useUserOrders({ page, limit: 10 });
    ...
    <PageTransition>
      ...
      <div className="bg-[#FAF8F4] px-6 py-12">
        <div className="mx-auto max-w-5xl">
          <h1 className="font-serif text-3xl text-[#14151A]">My orders</h1>

          {isLoading && (
            <p className="mt-12 text-center text-sm text-[#8B8B85]">Loading orders...</p>
          )}
            ...
            <>
              ...
              {meta && meta.totalPages > 1 && (
                <div className="mt-10 flex items-center justify-center gap-4">
                  <Button type="button" variant="secondary" disabled={!meta.hasPrevPage} onClick={() => updatePage(page - 1)}>
                    ← Previous
                  </Button>
                  <span className="text-sm text-[#8B8B85]">
                    Page <span className="font-medium text-[#14151A]">{meta.page}</span> of{" "}
                    <span className="font-medium text-[#14151A]">{meta.totalPages}</span>
                  </span>
                  <Button type="button" variant="secondary" disabled={!meta.hasNextPage} onClick={() => updatePage(page + 1)}>
                    Next →
                  </Button>
                </div>
              )}
